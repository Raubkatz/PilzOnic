...
def detect_binary_columns(X: pd.DataFrame) -> List[str]:
    """
    Detect binary columns, including neighbor-expanded variants.

    Rules:
    - Any column whose name matches a configured binary base name (including neighbor variants) is binary.
    - Additionally, any column that can be coerced to numeric and has values subset of {0,1} (ignoring NaN) is binary.
      This is a safety net if you change the dataset features later.
    """
    bin_cols = set()

    # name-based (neighbor variants)
    for base in BINARY_BASE_COLUMNS:
        for c in X.columns:
            if _is_neighbor_variant(c, base):
                bin_cols.add(c)

    # value-based fallback (robust to future feature changes)
    for c in X.columns:
    ...
    return [c for c in X.columns if c in bin_cols]  # preserve column order
...
def detect_categorical_columns(X: pd.DataFrame) -> List[str]:
    ...
    - Binary columns are explicitly excluded from categorical detection (even if stored as object/category).
    """
    cat_cols = set()
    bin_cols = set(detect_binary_columns(X))
...
def preprocess_features(X: pd.DataFrame) -> Tuple[pd.DataFrame, List[str]]:
    ...
    - Detect binary columns (including neighbor variants) and force them to numeric 0/1 (nullable Int64).
      This ensures they are used as REAL numeric features (and appear in feature importance).
    ...
    bin_cols = detect_binary_columns(X_out)
    cat_cols = detect_categorical_columns(X_out)

    # Binary: force to numeric 0/1 (keep as numeric feature, not categorical)
    for c in bin_cols:
        s = X_out[c]
        if pd.api.types.is_bool_dtype(s):
            X_out[c] = s.astype(int)
        else:
            s2 = s
            if s2.dtype == object or pd.api.types.is_categorical_dtype(s2):
            ...
            # keep only {0,1,NA}; anything else becomes NA
            s_num = s_num.where(s_num.isna() | s_num.isin([0, 1]), other=pd.NA)
            X_out[c] = s_num.astype("Int64")
    ...
    # Boolean -> int (for any remaining bool columns)
    for c in X_out.columns:
        if pd.api.types.is_bool_dtype(X_out[c]):
            X_out[c] = X_out[c].astype(int)

    # Non-categorical and non-binary: numeric coercion
...
def train_one_target(
    ...
    fi_df = pd.DataFrame({"feature": list(X.columns), "importance": importances}).sort_values("importance",
                                                                                              ascending=False)
